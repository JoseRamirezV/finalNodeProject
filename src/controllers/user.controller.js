...
module.exports = {
   login: async (req, res) => {
      try {
         ...
         if (!user)
            throw new Error(
               'Este correo no esta asociado con una cuenta Taskty'
            );
   ...
   signup: async (req, res) => {
      try {
         ...
         if (exists)
            return res.json({
               error: 'Este correo electrónico ya se encuentra asociado a una cuenta Taskty',
            });
   ...
   update: async (req, res) => {
      try {
         ...
         if (needsVerification) {
            ...
            if (emailExists) {
               throw new Error(
                  'Este email ya se encuentra registrado en Taskty'
               );
            }
         ...
         if (needsVerification) {
            const { temporalToken, verificationUrl, oldEmail } =
               needsVerification;
            await sendEmail(
               newData.email,
            ...
            const tasksToNotifyToday = await Fact.find({
               userEmail: newData.email,
               notify: true,
               notificationDate: {
               ...
               for (const {
                  ...
                  scheduleJob(`${_id}`, date, async () => {
                     await sendEmail(userEmail, title, 'Notification', {
                        description,
                        limitDate: moment(limitDate).format(
                           'DD [de] MMMM [del] YYYY'
                        ),
                     });
   ...
   delete: async (req, res) => {
      try {
         ...
         await Fact.deleteMany({ userEmail: email });
   ...
   forgotPassword: async (req, res) => {
      try {
         ...
         if (temporalToken) {
            ...
            if (!userData)
               throw new Error(
                  'Este correo no se encuentra registrado en Taskty'
               );
         ...
         res.json({ ok: 'Contraseña actualizada', user: found });
         return;
      } catch (error) {
         res.status(500).json({ error: error.message });
      }
   ...
   verifyAccount: async (req, res) => {
      try {
         ...
         if (verified)
            throw new Error('Este usuario ya se encuentra verificado');
